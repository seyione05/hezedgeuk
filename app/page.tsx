...
const selectedExperience = [
  { name: "JaSure", href: "https://jasure.ca/", description: "On-demand insurance platform" },
  { name: "KOY Immigration", href: "https://koyimmigration.com/", description: "Canadian immigration services" },
  { name: "DLA Online", href: "https://dlaonline.org/", description: "Digital platform and online services" },
  { name: "SB Visas", href: "https://sbvisas.com/", description: "Visa and immigration support" },
  { name: "Association of Corporate Trustees", href: "https://corporatetrustees.org.ng/", description: "Professional association for corporate trustees" },
  { name: "Aspen Healthcare Staffing", href: "https://aspenhealthcarestaffing.com/", description: "Healthcare staffing services" },
];
...
export default function Home(){return <main>
  ...
  <section className="section" id="work"><div className="shell"><div className="section-heading"><div><p className="eyebrow">Selected experience</p><h2>Work grounded in real business needs.</h2></div><p>HezEdge has supported organisations in insurance, immigration, digital services, professional bodies and healthcare.</p></div><div className="work-grid">{selectedExperience.map((project,index)=><a className={`work-card work-${(index%4)+1}`} key={project.href} href={project.href} target="_blank" rel="noreferrer"><span>0{index+1}</span><h3>{project.name}</h3><p>{project.description}</p></a>)}</div></div></section>
  <section className="section section-soft" id="testimonials"><div className="shell"><div className="section-heading"><div><p className="eyebrow">Client perspective</p><h2>Partnership without the technical fog.</h2></div></div><div className="quote-grid">{testimonials.map(q=><figure key={q.name}><blockquote>“{q.quote}”</blockquote><figcaption><strong>{q.name}</strong><span>{q.context}</span></figcaption></figure>)}</div></div></section>
